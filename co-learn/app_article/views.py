...
#     article_model = LearningSpace.objects.get(id=article_id)
#     if req.method == "POST":
#         form = Article(req.POST, instance=article_model)
#         if form.is_valid():
#             form.save()
#             return HttpResponseRedirect(f"/learning-space/{article_model.id}")
#     else:
#         article_as_dict = model_to_dict(article_model)
#         form = Article(initial=article_as_dict)
#
